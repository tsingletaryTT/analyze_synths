...
class MarkdownExporter:
    ...
    def generate_comprehensive_report(self, df: pd.DataFrame, phase_data: List[Dict[str, Any]],
                                    ...
                                    narrative_results: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        Args:
            ...
            narrative_results: Dict mapping filename -> NarrativeResult, from
                ParallelAudioAnalyzer.narrative_results (optional). When present,
                a "Temporal Narrative" section is appended summarising each
                track's arc and closest neighbours.
        ...
        try:
            ...
            # ---- Narrative section (if narrative_results available) ----
            if narrative_results:
                report_sections.append(self._generate_narrative_section(narrative_results))
...
5. **Character Blending**: Consider combining your dominant character types for richer textures"""
    ...
    def _generate_narrative_section(self, narrative_results: Dict[str, Any]) -> str:
        """
        Generate the Temporal Narrative section from NarrativeResult objects.

        Each entry in `narrative_results` maps a filename (or track identifier)
        to a NarrativeResult (or any object that exposes `.narrative` and
        `.similar_to` attributes).  The section surfaces the prose narrative
        generated by NarrativeAnalyzer and, when cross-piece similarity data is
        present, lists the nearest-neighbour tracks.

        Args:
            narrative_results: Dict[str, NarrativeResult] produced by
                ParallelAudioAnalyzer after analyze_directory().

        Returns:
            Markdown string for the Temporal Narrative section.
        """
        lines: List[str] = []
        lines.append("## Temporal Narrative\n")
        lines.append(
            "*An AI-generated prose description of how each track evolves over time, "
            "plus the tracks it resembles most closely.*\n"
        )

        for filename, result in narrative_results.items():
            lines.append(f"### {filename}\n")

            # narrative attribute holds the prose string
            narrative_text = getattr(result, "narrative", None) or ""
            if narrative_text:
                lines.append(narrative_text + "\n")
            else:
                lines.append("*No narrative available for this track.*\n")

            # similar_to is an optional list of filename strings
            similar_to = getattr(result, "similar_to", None)
            if similar_to:
                similar_str = ", ".join(str(s) for s in similar_to)
